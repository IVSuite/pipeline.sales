...
interface ImportSummary {
  ...
  skipped: number;
  failed: number;
  errors: { index: number; name: string; key: string; message: string }[];
}

const BATCH_SIZE = 100;

/**
 * Generic bulk-import wizard (Upload → Map → Preview → Import → Summary).
 * Driven entirely by `config`; Customers, Leads, and Companies all render this
 * same component with a different field set / endpoint.
 */
export function ImportDialog({
  ...
  const emptyMapping = useMemo<ColumnMapping>(() => {
    const m: ColumnMapping = {};
    for (const f of config.fields) m[f.key] = null;
    return m;
  }, [config.fields]);
  ...
  const displayField = useMemo(
    () => config.fields.find((f) => f.required)?.key ?? config.fields[0].key,
    [config.fields]
  );
  const displayLabel = useMemo(
    () => config.fields.find((f) => f.key === displayField)?.label ?? "Name",
    [config.fields, displayField]
  );
  // "Full name is required — map it to continue." for Customers/Leads,
  // "Company name is required — …" for Companies. Wording is unchanged from
  // the original Customers dialog.
  const requiredLabels = useMemo(
    () => config.fields.filter((f) => f.required).map((f) => f.label),
    [config.fields]
  );
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch(`${config.endpoint}/parse`, { method: "POST", body: form });
        ...
        setParsed(data);
        setMapping({ ...emptyMapping, ...data.suggestedMapping });
        setStep("map");
      } catch (e) {
    ...
    [config.endpoint, emptyMapping]
  );

  const downloadTemplate = useCallback(async () => {
    try {
      const res = await fetch(`${config.endpoint}/template`);
  ...
  }, [config.endpoint, config.templateFileName]);
  ...
  const previewRows = useMemo<PreviewRow[]>(() => {
    ...
    return parsed.rows.map((row, i) => {
      const mapped = applyMapping(row, mapping, config.fields);
      const v = validateMappedRow(mapped, config.fields);
      const normKey = normalizeDedupeKey(v.data[config.dedupeField]);
      const duplicateInFile = normKey ? seenKeys.has(normKey) : false;
  ...
  }, [parsed, mapping, config.fields, config.dedupeField]);
  ...
  const requiredMapped = config.fields.every((f) => !f.required || mapping[f.key] !== null);
  ...
  const runImport = useCallback(async () => {
    ...
    const agg: ImportSummary = {
      ...
      errors: invalidPreview.map((r) => ({
        index: r.rowNumber,
        name: r.values[displayField] ?? "",
        key: r.values[config.dedupeField] ?? "",
      ...
      for (let i = 0; i < validPreview.length; i += BATCH_SIZE) {
        ...
        const res = await fetch(config.endpoint, {
          ...
          body: JSON.stringify({
            ...
            rows: slice.map((r) => {
              ...
              for (const f of config.fields) obj[f.key] = r.values[f.key] ?? "";
              return obj;
            }),
          }),
        });
        ...
        for (const err of json.errors ?? []) {
          ...
          agg.errors.push({
            ...
            // Tolerant of both the shared runner shape {name,key} and the
            // Customers route shape {full_name,email}.
            name: err.name ?? err.full_name ?? "",
            key: err.key ?? err.email ?? "",
            message: err.message,
          });
        }
  ...
  }, [parsed, previewRows, duplicateMode, queryClient, config, displayField]);

  const downloadFailedRows = useCallback(() => {
    if (!summary || summary.errors.length === 0) return;
    const header = ["Row", displayLabel, capitalize(config.dedupeLabel), "Error"];
    const lines = [header, ...summary.errors.map((e) => [String(e.index), e.name, e.key, e.message])];
    const csv = lines.map((cols) => cols.map(csvCell).join(",")).join("\r\n");
    triggerDownload(new Blob([csv], { type: "text/csv;charset=utf-8" }), `failed-${config.entityNounPlural}-rows.csv`);
  }, [summary, config.dedupeLabel, config.entityNounPlural, displayLabel]);
        ...
        <div className="space-y-4">
          ...
          {!requiredMapped && (
            <p className="rounded-lg bg-red-500/10 px-3 py-2 text-xs text-red-500">
              {requiredLabels.join(", ")} {requiredLabels.length === 1 ? "is" : "are"} required — map{" "}
              {requiredLabels.length === 1 ? "it" : "them"} to continue.
            </p>
          )}
        ...
        <div className="space-y-4">
                    ...
                    <tr key={r.rowNumber} className="border-t border-border">
                      ...
                      {config.fields.map((f) => (
                        <td key={f.key} className="px-2 py-1.5">
                          {r.values[f.key] ||
                            (f.key === displayField ? <em className="text-muted-foreground">—</em> : "—")}
                        </td>
                      ))}
          ...
          <div className="rounded-lg bg-surface-muted p-3">
            <p className="mb-2 text-sm font-medium">
              When a {config.entityNoun} with the same {config.dedupeLabel} already exists:
            </p>
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                Update the existing {config.entityNoun}
              </label>
            </div>
          </div>
          ...
          <div className="flex justify-between pt-1">
            ...
            <Button onClick={runImport} disabled={stats.valid === 0}>
              Import {stats.valid} {stats.valid === 1 ? config.entityNoun : config.entityNounPlural}{" "}
        ...
        <div className="space-y-4">
          ...
          {summary.skipped > 0 && (
            <p className="text-xs text-muted-foreground">
              Skipped rows were duplicates of existing {config.entityNounPlural} (matched by {config.dedupeLabel}) and
              were left untouched.
            </p>
          )}
